...
func _on_visible_on_screen_enabler_2d_screen_exited() -> void:
	get_tree().root.get_node("Main/BugSpawner").increase_resistance(type, misuse_res_contribution)
	queue_free()


func _on_area_entered(area: Area2D) -> void:
	get_tree().root.get_node("Main/BugSpawner").increase_resistance(type, res_contribution)
